...
export async function eip2612Permit({ token, chain, ownerAddress, spenderAddress, value }: { token: any; chain: any; ownerAddress: any; spenderAddress: any; value: any }) {
  return {
    types: {
      Permit: [
    ...
    message: {
      ...
      deadline: maxUint256, // The paymaster cannot access block.timestamp due to 4337 opcode restrictions, so the deadline must be MAX_UINT256.
    },
  };
}
